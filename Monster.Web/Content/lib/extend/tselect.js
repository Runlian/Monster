layui.define(['table', 'jquery', 'form'], function (exports) {
    ...
    * 初始化表格选择器
    */
    TSelect.prototype.render = function (opt) {
        var elem = $(opt.elem);
        ...
        elem.off('click').on('click', function (e) {
            ...
            if ($('div.tselect').length >= 1) {
                return false;
            }

            var t = elem.offset().top + elem.outerHeight() + "px";
            var l = elem.offset().left + "px";
            var tableName = "tselect_table_" + new Date().getTime();
            var tableBox = '<div class="tselect layui-anim layui-anim-upbit" style="left:' + l + ';top:' + t + ';border: 1px solid #d2d2d2;background-color: #fff;box-shadow: 0 2px 4px rgba(0,0,0,.12);padding:10px 10px 0 10px;position: absolute;z-index:66666666;margin: 5px 0;border-radius: 2px;min-width:530px;">';
            tableBox += '<div class="tselectBar">';
            tableBox += '<form class="layui-form" action="" style="display:inline-block;">';
            tableBox += '<input style="display:inline-block;width:190px;height:36px;vertical-align:middle;margin-right:-1px;border: 1px solid #C9C9C9;border-radius:0;" type="text" name="' + opt.searchKey + '" placeholder="' + opt.searchPlaceholder + '" autocomplete="off" class="layui-input"><button class="layui-btn layui-btn-sm layui-btn-primary tselect_btn_search" lay-submit lay-filter="tselect_btn_search"><i class="layui-icon layui-icon-search"></i></button>';
            tableBox += '</form>';
            tableBox += '<button style="float:right;padding:0 15px;" class="layui-btn layui-btn-sm tselect_btn_select">选择<span></span></button>';
            tableBox += '</div>';
            ...
            var tselect_table = table.render(opt.table);
            ...
            function updataButton(n) {
                tableBox.find('.tselect_btn_select span').html(n == 0 ? '' : '(' + n + ')')
            }
            ...
            //关键词搜索
            form.on('submit(tselect_btn_search)', function (data) {
                tselect_table.reload({
                    where: data.field,
                    page: {
                        curr: 1
                    }
            ...
            //按钮选中
            tableBox.find('.tselect_btn_select').on('click', function () {
                var checkStatus = table.checkStatus(tableName);
            ...
            $(document).mouseup(function (e) {
                var userSet_con = $('' + opt.elem + ',.tselect');
    ...
    * 隐藏选择器
    */
    TSelect.prototype.hide = function (opt) {
        $('.tselect').remove();
    }

    //自动完成渲染
    var tselect = new TSelect();
